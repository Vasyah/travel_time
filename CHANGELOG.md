Дата: 01.10.2025 Исправлены ошибки типизации и билда

Описание: Исправлены множественные ошибки типизации и билда в проекте:

1. **FormMultipleSelector.tsx**: Исправлен конфликт типов `Option` - удален локальный интерфейс и используется экспортируемый тип из `multiple-selector.tsx`.

2. **HotelInfo.tsx**:

    - Добавлено поле `rating` в схему валидации `hotelFormSchema`
    - Исправлена обработка `undefined` значений в `deserializeData` функции
    - Добавлена безопасная обработка массивов с `filter(Boolean)`

3. **RoomInfo.tsx**: Исправлена типизация `INITIAL_ROOM_FEATURES` для совместимости с типом `Option` из `multiple-selector.tsx`.

4. **RoomModal.tsx**: Исправлена типизация `onAccept` обработчиков с явным приведением типов.

5. **Search.tsx**: Исправлена типизация `field.value` для работы с массивом строк.

6. **adaptHotel.ts**: Заменен импорт `rc-select/es/Select` на `Option` из `multiple-selector.tsx`, переименована функция `adaptToAntOption` в `adaptToMultipleSelectorOption`.

7. **HotelTitle.tsx**:

    - Удалены неподдерживаемые размеры из типа `size`
    - Заменен пропс `transform` на CSS класс `uppercase`

8. **PhoneInputDeprecated.tsx**: Заменен неподдерживаемый пропс `rightSide` на обертку с абсолютным позиционированием иконок.

9. **theme.ts**: Удален неиспользуемый файл с зависимостью от `@consta/uikit`.

Все ошибки TypeScript устранены, билд проходит успешно. Проект готов к продакшену.
