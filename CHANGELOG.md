...
Дата: 21.09.2025 Полная миграция на Shadcn UI + Lucide Icons + InfoCard

Описание: Проведена полная миграция всех UI библиотек на Shadcn UI с переходом на Lucide Icons. **Consta UI Kit миграция**: Заменены все компоненты (Text→Typography, Button, Card, Modal→Dialog, Loader, User→Avatar+Badge, DateTime→Calendar, Tag→Badge, ResponsesNothingFound→EmptyState) в 25+ файлах. **Ant Design миграция**: Заменены Tabs→Shadcn Tabs в авторизации, Flex→Tailwind CSS в меню и layout, Button→Shadcn Button, Popconfirm→AlertDialog. **Новые компоненты**: Создан премиум InfoCard в `features/Main/ui/InfoCard.tsx` с анимациями чисел, градиентным фоном, hover-эффектами, контрастными Lucide иконками и крупной типографикой. Обновлена главная страница с использованием Building2, Key, Calendar иконок. **Технические улучшения**: HotelsTable переведен на @tanstack/react-table, добавлены Tooltip для длинных названий, создана качественная мобильная адаптация, добавлен маппинг значений в русские названия. Полная миграция завершена с современным UI, отличной мобильной адаптацией, полной локализацией и премиум-дизайном.
