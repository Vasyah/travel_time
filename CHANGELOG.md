...
Дата: 21.09.2025 Полная миграция на Shadcn UI: Consta UI Kit + Ant Design → Shadcn UI

Описание: Проведена полная миграция всех UI библиотек на Shadcn UI. **Consta UI Kit миграция**: Заменены все компоненты (Text→Typography, Button, Card, Modal→Dialog, Loader, User→Avatar+Badge, DateTime→Calendar, Tag→Badge, ResponsesNothingFound→EmptyState) в 25+ файлах. **Ant Design миграция**: Заменены Tabs→Shadcn Tabs в странице авторизации, Flex→Tailwind CSS в меню и страницах, Button→Shadcn Button в Room компоненте, Popconfirm→AlertDialog в ConfirmButton. **Технические улучшения**: HotelsTable переведен на @tanstack/react-table, добавлены Tooltip для длинных названий, создана качественная мобильная адаптация с карточками, добавлен маппинг значений в русские названия (getValueLabel, VALUE_TO_LABEL_MAP). Полная миграция завершена с современным UI, отличной мобильной адаптацией и полной локализацией.
