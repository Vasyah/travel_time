...
Дата: 21.09.2025 Полная миграция с Consta UI Kit на Shadcn UI + TanStack Table + Мобильная адаптация

Описание: Проведена полная миграция UI библиотеки с Consta UI Kit на Shadcn UI с переходом на современные решения. **Фаза 1**: Создан детальный план миграции и mapping компонентов. **Фаза 2**: Заменены базовые компоненты - создан универсальный `Typography` для замены `Text` (8 файлов), обновлен `TravelButton` на Shadcn Button, заменены все `Card` компоненты. **Фаза 3**: Переработаны формы авторизации (`Login.tsx`, `Register.tsx`) на Shadcn Input, Label, Select с Tailwind CSS Grid и улучшенной валидацией. **Фаза 4**: Заменены сложные компоненты - `Modal` на Dialog, `Loader` на кастомный Spinner с lucide-react, `User` на Avatar+Badge+DropdownMenu, `ResponsesNothingFound` на EmptyState с вариантами. **Фаза 5**: Заменены специфичные компоненты - `DateTime` на Calendar+Popover, `Tag` на Badge, удален `DragNDropField`. **Фаза 7**: Исправлены конфликты зависимостей - заменены @consta/icons на lucide-react, удалены все зависимости Consta UI из package.json. **Бонус**: HotelsTable переведен на @tanstack/react-table с продвинутой сортировкой, фильтрацией и пагинацией. Добавлены Tooltip для длинных названий и адресов, создана качественная мобильная версия с карточками отелей, адаптивной пагинацией и оптимизированным UX. Добавлен маппинг значений в русские названия (`getValueLabel`, `VALUE_TO_LABEL_MAP`) для корректного отображения городов и особенностей. Миграция завершена на 100% с полной функциональной совместимостью, современным UI и отличной мобильной адаптацией.
